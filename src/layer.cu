...
__global__  __launch_bounds__(256) void sgemm_large(int M, int N, int K, float *A, float *B, float *b,float *C){
    ...
    *(((float4*)buffer_B) + 2 * tx + 1) = prefetch_vector_tile_B[1];
    ...
    C += (by * ns + offset_vec_B_warp + offset_vec_B_thread) * M;
    int bias_start = bx * ms + offset_vec_A_warp + offset_vec_A_thread;
    float4 bias_vec_1 = *((float4*)(b + bias_start));
    float4 bias_vec_2 = *((float4*)(b + bias_start + 4));
    ...
    C_res[0].x = res[0 ] + bias_vec_1.x;
    C_res[0].y = res[8 ] + bias_vec_1.y;
    C_res[0].z = res[16] + bias_vec_1.z;
    C_res[0].w = res[24] + bias_vec_1.w;

    C_res[1].x = res[32] + bias_vec_2.x;
    C_res[1].y = res[40] + bias_vec_2.y;
    C_res[1].z = res[48] + bias_vec_2.z;
    C_res[1].w = res[56] + bias_vec_2.w;

    C_res[2].x = res[1 ] + bias_vec_1.x;
    C_res[2].y = res[9 ] + bias_vec_1.y;
    C_res[2].z = res[17] + bias_vec_1.z;
    C_res[2].w = res[25] + bias_vec_1.w;

    C_res[3].x = res[33] + bias_vec_2.x;
    C_res[3].y = res[41] + bias_vec_2.y;
    C_res[3].z = res[49] + bias_vec_2.z;
    C_res[3].w = res[57] + bias_vec_2.w;

    C_res[4].x = res[2 ] + bias_vec_1.x;
    C_res[4].y = res[10] + bias_vec_1.y;
    C_res[4].z = res[18] + bias_vec_1.z;
    C_res[4].w = res[26] + bias_vec_1.w;

    C_res[5].x = res[34] + bias_vec_2.x;
    C_res[5].y = res[42] + bias_vec_2.y;
    C_res[5].z = res[50] + bias_vec_2.z;
    C_res[5].w = res[58] + bias_vec_2.w;

    C_res[6].x = res[3 ] + bias_vec_1.x;
    C_res[6].y = res[11] + bias_vec_1.y;
    C_res[6].z = res[19] + bias_vec_1.z;
    C_res[6].w = res[27] + bias_vec_1.w;

    C_res[7].x = res[35] + bias_vec_2.x;
    C_res[7].y = res[43] + bias_vec_2.y;
    C_res[7].z = res[51] + bias_vec_2.z;
    C_res[7].w = res[59] + bias_vec_2.w;

    C_res[8].x = res[4 ] + bias_vec_1.x;
    C_res[8].y = res[12] + bias_vec_1.y;
    C_res[8].z = res[20] + bias_vec_1.z;
    C_res[8].w = res[28] + bias_vec_1.w;

    C_res[9].x = res[36] + bias_vec_2.x;
    C_res[9].y = res[44] + bias_vec_2.y;
    C_res[9].z = res[52] + bias_vec_2.z;
    C_res[9].w = res[60] + bias_vec_2.w;

    C_res[10].x = res[5 ] + bias_vec_1.x;
    C_res[10].y = res[13] + bias_vec_1.y;
    C_res[10].z = res[21] + bias_vec_1.z;
    C_res[10].w = res[29] + bias_vec_1.w;

    C_res[11].x = res[37] + bias_vec_2.x;
    C_res[11].y = res[45] + bias_vec_2.y;
    C_res[11].z = res[53] + bias_vec_2.z;
    C_res[11].w = res[61] + bias_vec_2.w;

    C_res[12].x = res[6 ] + bias_vec_1.x;
    C_res[12].y = res[14] + bias_vec_1.y;
    C_res[12].z = res[22] + bias_vec_1.z;
    C_res[12].w = res[30] + bias_vec_1.w;

    C_res[13].x = res[38] + bias_vec_2.x;
    C_res[13].y = res[46] + bias_vec_2.y;
    C_res[13].z = res[54] + bias_vec_2.z;
    C_res[13].w = res[62] + bias_vec_2.w;

    C_res[14].x = res[7 ] + bias_vec_1.x;
    C_res[14].y = res[15] + bias_vec_1.y;
    C_res[14].z = res[23] + bias_vec_1.z;
    C_res[14].w = res[31] + bias_vec_1.w;

    C_res[15].x = res[39] + bias_vec_2.x;
    C_res[15].y = res[47] + bias_vec_2.y;
    C_res[15].z = res[55] + bias_vec_2.z;
    C_res[15].w = res[63] + bias_vec_2.w;
...
__global__  __launch_bounds__(256) void sgemm_large_relu(int M, int N, int K, float *A, float *B, float *b,float *C){
    // ms = ns = 64, ks = 8
    // mw = 32, nw = 64
    // mr = 8, nr = 8
    ...
    *(((float4*)buffer_B) + 2 * tx + 1) = prefetch_vector_tile_B[1];
    ...
    float tmp;
    tmp = res[0 ] + bias_vec_1.x;
    C_res[0].x = tmp > 0 ? tmp : 0;
    tmp = res[8 ] + bias_vec_1.y;
    C_res[0].y = tmp > 0 ? tmp : 0;
    tmp = res[16] + bias_vec_1.z;
    C_res[0].z = tmp > 0 ? tmp : 0;
    tmp = res[24] + bias_vec_1.w;
    C_res[0].w = tmp > 0 ? tmp : 0;

    tmp = res[32] + bias_vec_2.x;
    C_res[1].x = tmp > 0 ? tmp : 0;
    tmp = res[40] + bias_vec_2.y;
    C_res[1].y = tmp > 0 ? tmp : 0;
    tmp = res[48] + bias_vec_2.z;
    C_res[1].z = tmp > 0 ? tmp : 0;
    tmp = res[56] + bias_vec_2.w;
    C_res[1].w = tmp > 0 ? tmp : 0;

    tmp = res[1 ] + bias_vec_1.x;
    C_res[2].x = tmp > 0 ? tmp : 0;
    tmp = res[9 ] + bias_vec_1.y;
    C_res[2].y = tmp > 0 ? tmp : 0;
    tmp = res[17] + bias_vec_1.z;
    C_res[2].z = tmp > 0 ? tmp : 0;
    tmp = res[25] + bias_vec_1.w;
    C_res[2].w = tmp > 0 ? tmp : 0;

    tmp = res[33] + bias_vec_2.x;
    C_res[3].x = tmp > 0 ? tmp : 0;
    tmp = res[41] + bias_vec_2.y;
    C_res[3].y = tmp > 0 ? tmp : 0;
    tmp = res[49] + bias_vec_2.z;
    C_res[3].z = tmp > 0 ? tmp : 0;
    tmp = res[57] + bias_vec_2.w;
    C_res[3].w = tmp > 0 ? tmp : 0;

    tmp = res[2 ] + bias_vec_1.x;
    C_res[4].x = tmp > 0 ? tmp : 0;
    tmp = res[10] + bias_vec_1.y;
    C_res[4].y = tmp > 0 ? tmp : 0;
    tmp = res[18] + bias_vec_1.z;
    C_res[4].z = tmp > 0 ? tmp : 0;
    tmp = res[26] + bias_vec_1.w;
    C_res[4].w = tmp > 0 ? tmp : 0;

    tmp = res[34] + bias_vec_2.x;
    C_res[5].x = tmp > 0 ? tmp : 0;
    tmp = res[42] + bias_vec_2.y;
    C_res[5].y = tmp > 0 ? tmp : 0;
    tmp = res[50] + bias_vec_2.z;
    C_res[5].z = tmp > 0 ? tmp : 0;
    tmp = res[58] + bias_vec_2.w;
    C_res[5].w = tmp > 0 ? tmp : 0;

    tmp = res[3 ] + bias_vec_1.x;
    C_res[6].x = tmp > 0 ? tmp : 0;
    tmp = res[11] + bias_vec_1.y;
    C_res[6].y = tmp > 0 ? tmp : 0;
    tmp = res[19] + bias_vec_1.z;
    C_res[6].z = tmp > 0 ? tmp : 0;
    tmp = res[27] + bias_vec_1.w;
    C_res[6].w = tmp > 0 ? tmp : 0;

    tmp = res[35] + bias_vec_2.x;
    C_res[7].x = tmp > 0 ? tmp : 0;
    tmp = res[43] + bias_vec_2.y;
    C_res[7].y = tmp > 0 ? tmp : 0;
    tmp = res[51] + bias_vec_2.z;
    C_res[7].z = tmp > 0 ? tmp : 0;
    tmp = res[59] + bias_vec_2.w;
    C_res[7].w = tmp > 0 ? tmp : 0;

    tmp = res[4 ] + bias_vec_1.x;
    C_res[8].x = tmp > 0 ? tmp : 0;
    tmp = res[12] + bias_vec_1.y;
    C_res[8].y = tmp > 0 ? tmp : 0;
    tmp = res[20] + bias_vec_1.z;
    C_res[8].z = tmp > 0 ? tmp : 0;
    tmp = res[28] + bias_vec_1.w;
    C_res[8].w = tmp > 0 ? tmp : 0;

    tmp = res[36] + bias_vec_2.x;
    C_res[9].x = tmp > 0 ? tmp : 0;
    tmp = res[44] + bias_vec_2.y;
    C_res[9].y = tmp > 0 ? tmp : 0;
    tmp = res[52] + bias_vec_2.z;
    C_res[9].z = tmp > 0 ? tmp : 0;
    tmp = res[60] + bias_vec_2.w;
    C_res[9].w = tmp > 0 ? tmp : 0;

    tmp = res[5 ] + bias_vec_1.x;
    C_res[10].x = tmp > 0 ? tmp : 0;
    tmp = res[13] + bias_vec_1.y;
    C_res[10].y = tmp > 0 ? tmp : 0;
    tmp = res[21] + bias_vec_1.z;
    C_res[10].z = tmp > 0 ? tmp : 0;
    tmp = res[29] + bias_vec_1.w;
    C_res[10].w = tmp > 0 ? tmp : 0;

    tmp = res[37] + bias_vec_2.x;
    C_res[11].x = tmp > 0 ? tmp : 0;
    tmp = res[45] + bias_vec_2.y;
    C_res[11].y = tmp > 0 ? tmp : 0;
    tmp = res[53] + bias_vec_2.z;
    C_res[11].z = tmp > 0 ? tmp : 0;
    tmp = res[61] + bias_vec_2.w;
    C_res[11].w = tmp > 0 ? tmp : 0;

    tmp = res[6 ] + bias_vec_1.x;
    C_res[12].x = tmp > 0 ? tmp : 0;
    tmp = res[14] + bias_vec_1.y;
    C_res[12].y = tmp > 0 ? tmp : 0;
    tmp = res[22] + bias_vec_1.z;
    C_res[12].z = tmp > 0 ? tmp : 0;
    tmp = res[30] + bias_vec_1.w;
    C_res[12].w = tmp > 0 ? tmp : 0;

    tmp = res[38] + bias_vec_2.x;
    C_res[13].x = tmp > 0 ? tmp : 0;
    tmp = res[46] + bias_vec_2.y;
    C_res[13].y = tmp > 0 ? tmp : 0;
    tmp = res[54] + bias_vec_2.z;
    C_res[13].z = tmp > 0 ? tmp : 0;
    tmp = res[62] + bias_vec_2.w;
    C_res[13].w = tmp > 0 ? tmp : 0;

    tmp = res[7 ] + bias_vec_1.x;
    C_res[14].x = tmp > 0 ? tmp : 0;
    tmp = res[15] + bias_vec_1.y;
    C_res[14].y = tmp > 0 ? tmp : 0;
    tmp = res[23] + bias_vec_1.z;
    C_res[14].z = tmp > 0 ? tmp : 0;
    tmp = res[31] + bias_vec_1.w;
    C_res[14].w = tmp > 0 ? tmp : 0;

    tmp = res[39] + bias_vec_2.x;
    C_res[15].x = tmp > 0 ? tmp : 0;
    tmp = res[47] + bias_vec_2.y;
    C_res[15].y = tmp > 0 ? tmp : 0;
    tmp = res[55] + bias_vec_2.z;
    C_res[15].z = tmp > 0 ? tmp : 0;
    tmp = res[63] + bias_vec_2.w;
    C_res[15].w = tmp > 0 ? tmp : 0;
